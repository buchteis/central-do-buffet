...
-- Backfill: ressincroniza TODOS os eventos ativos de TODOS os buffets
DO $$
DECLARE e RECORD;
BEGIN
  FOR e IN SELECT id FROM public.events
            WHERE status::text IN ('agendado','em_andamento','pagamento_parcial','pago','realizado','concluido')
            ORDER BY event_date
  LOOP
    PERFORM public.sync_event_stock(e.id);
  END LOOP;
END $$;