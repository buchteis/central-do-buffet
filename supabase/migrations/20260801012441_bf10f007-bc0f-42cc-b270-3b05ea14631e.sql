...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'package_id', pk.id,
           'name', pk.name,
           'price_per_person', px.price
         )), '[]'::jsonb),
         COALESCE(SUM(px.price), 0)
    INTO v_packages, v_ppp
  FROM public.packages pk
  CROSS JOIN LATERAL (
    SELECT COALESCE(
      (SELECT t.price_per_person
         FROM public.package_price_tiers t
        WHERE t.package_id = pk.id
          AND v_guests >= t.min_guests AND v_guests <= t.max_guests
        ORDER BY t.position
        LIMIT 1),
      (SELECT t.price_per_person
         FROM public.package_price_tiers t
        WHERE t.package_id = pk.id
        ORDER BY abs(t.min_guests - v_guests)
        LIMIT 1),
      pk.price_per_person,
      0
    ) AS price
