...
BEGIN
  ...
  -- Itens unitários primeiro (necessário para remover pacotes duplicados sem preço)
  ...
  -- Preço por pessoa: faixa exata; fora das faixas, faixa mais próxima abaixo/acima
  WITH pkg AS (
    SELECT pk.id, pk.name,
           COALESCE(
             (SELECT t.price_per_person FROM public.package_price_tiers t
               ...
               ORDER BY t.position, t.min_guests LIMIT 1),
             (SELECT t.price_per_person FROM public.package_price_tiers t
               WHERE t.package_id = pk.id AND t.max_guests < v_guests
               ORDER BY t.max_guests DESC LIMIT 1),
             (SELECT t.price_per_person FROM public.package_price_tiers t
               WHERE t.package_id = pk.id AND t.min_guests > v_guests
               ORDER BY t.min_guests ASC LIMIT 1),
             pk.price_per_person,
             0
           ) AS price
